...
public class Flocker : Vehicle
{
    ...
    protected override void CalcSteeringForces()
    {
        ...
        total += 1.5f*Separate(FlockManager.instance.flock, 2);
        total += Align(FlockManager.instance.flock, 1);
        total += Cohere(FlockManager.instance.flock, .5f);
        total += Wander(3, 2);
        total -= gameObject.transform.position / 25;
        foreach(GameObject obstacle in ObstacleManager.instance.obstacles)
        {
            total += AvoidObstacle(obstacle);
        }
        total.y = 0;
